...
func TestFloat64(t *testing.T) {
	...
	for _, negative := range []bool{false, true} {
		...
		Convey(fmt.Sprintf("a %s float64", negdesc), t, func() {

			var a float64 = float64(rand.Float64() * math.MaxInt32)
			if negative {
				a = -a
			}
			...
			for _, inverse := range []bool{false, true} {
				...
				Convey(fmt.Sprintf("can be encoded in %s order into a sufficient byte array", invdesc), func() {
					...
					Convey("and decoded into a float64 pointer", func() {
						var v float64
